...
class UpdateOrderStatusControllerTest {
    ...
    private OrderResponse generateResponse() {
        ...
        return new OrderResponse(orderId, "ORDER-21312", customerResponse, null, null, LocalDate.of(2023, 3, 19), LocalTime.of(8, 30), "MOTORCYCLE", "B 1234 ABA", null, null, null, OrderStatus.ACCEPTED, bookedTime, null, bookedTime.plusMinutes(5), null, null, null, null, null, "0");
    }
    ...
    void update_order_status() throws Exception {
        ...
        OrderResponse orderWithUpdatedStatus = generateResponse();
        BaseResponse<Object> response = BaseResponse.builder()
                .statusCode(200)
                .message("Order status updated to " + request.getStatus())
                .data(orderWithUpdatedStatus)
                .build();
        ...
        when(orderService.updateStatus(orderId, request)).thenReturn(orderWithUpdatedStatus);
        SecurityContextHolder.setContext(securityContext);

        String requestTarget = "/api/orders/" + orderId + "/status";
        mockMvc.perform(MockMvcRequestBuilders.patch(requestTarget)
