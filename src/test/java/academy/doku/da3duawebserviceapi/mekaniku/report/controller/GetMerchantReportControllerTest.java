...
@WebMvcTest(controllers = GetMerchantReportController.class)
@AutoConfigureMockMvc(addFilters = false)
@Import(WebMvcSecurityConfig.class)
class GetMerchantReportControllerTest {
    @Autowired
    MockMvc mockMvc;
    ...
    @Test
    @DisplayName("When get merchant report endpoint is hit, should return response with status code 200")
    void get_merchant_report() throws Exception {
        Integer merchantId = 1;

        Pageable pageable = PageRequest.of(0, 10);

        PaginationResponse<List<ReportResponse>> merchantReport = generateResponse();
        BaseResponse response = BaseResponse.builder()
                .statusCode(200)
                .message("Report list for Workshop with ID:" + merchantId)
                .data(merchantReport)
                .build();
        ...
        when(transactionReportService.getMerchantReport(pageable, merchantId)).thenReturn(merchantReport);
        SecurityContextHolder.setContext(securityContext);

        String requestTarget = "/api/workshops/" + merchantId + "/reports";
